...
const complex<double> c(-0.4, 0.6);

int getJuliaN(complex<double> c, complex<double> z)
{
...
void sequential()
{
    ...
    { // row
        ...
        { // the pixels in each row
            complex<double> z = mapPixelToComplexPlane(x, y);
            int n = getJuliaN(c, z);
...
void parallelStatic(const int size, const int rank)
{
    ...
    { // else if slave node -> process and return a color (int)
        ...
        for (int y = rows; y < (rank == size - 1 ? height : rows + rowIncrement); y++)
        {
            ...
            {
                complex<double> z = mapPixelToComplexPlane(x, y);
                int n = getJuliaN(c, z); // returns an N
                pixel[0] = n;
                pixel[1] = x;
                pixel[2] = y;
